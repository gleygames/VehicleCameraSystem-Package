# Step 02 — Single-body orbit

- **Difficulty:** Hard
- **Recommended agent:** Astra High (gpt-6-astra, high)
- **Why:** Distance parameterization, topology, reference frames, and regressions shared by composed curves.
- **Audit baseline:** Partial; confirmed sampler defect
- **Prerequisites:** [Step 00](./Step00-package-skeleton.md), [Step 01](./Step01-fixed-view.md)
- **Requirement IDs:** DATA-01, ORBIT-01, ORBIT-07, MOVE-01, INPUT-02. Shared IDs cover this step's slice only; see [coverage](../RequirementCoverage.md).

## Startup checkpoint

**MANDATORY HARD CHECKPOINT — STOP FIRST.** Read this task's guidance and difficulty, then say:

> This is a Hard task: Step 02 — Single-body orbit. Recommended agent: Astra High. Please set the agent you want to use, then explicitly tell me to continue. I am paused before implementation and tests.
...
First recovery task. The audit executed current geometry source: on the existing 20-by-10 rectangle, distance 20.01 evaluates near (20,0,9.69) instead of (20,0,0.01). ClosedBezierOrbit and LinearOrbitComposer both interpolate parameters across different segments incorrectly. Repair both sampler paths here without extending chain features.
...
Add a closed planar curve, topology validation, distance sampling, manual horizontal travel, per-view movement rate, and local-frame defaults with orientation adjustment.

Original Edit Mode check: Check closure, intersection, length, sampling, frame, and rate settings.

Original Play Mode check/observation: Orbit around short and five-times-long curves; observe gentle start and immediate stop on release.

## Scope and acceptance outcomes

- Repair physical-distance sampling on standalone and already-existing composed curves, including every segment boundary and loop wrap.
- Verify planar closed topology, intersection rejection, orientation adjustment, physical-speed manual travel, per-view rate, gentle start, and immediate stop.
- Use the existing Step 0/1 smoke tests as recovery entry evidence; their historical user acceptance is unknown. Do not silently treat them as accepted.
...
- Add the analytic rectangle regression at distances 19.99, 20, 20.01, 20.1, 20.3125 and 20.32; expected positions must be calculated independently of the sampler.
- Check just before/at/after every knot and connector boundary, negative distance and multiple wraps; verify continuity and forward progress on straight and curved fixtures.
- Use an independent high-resolution reference or analytic geometry for curved length/position tolerances; test invalid topology and nonidentity orientation.
...
- Drive an actual Camera through public intent commands across every boundary; assert no large jump or reversed motion for a small positive travel increment.
- Compare a curve with a uniformly five-times-longer curve at the same physical speed; compare full traversal times after accounting for start easing.
- Verify gentle start and immediate release stop at several delta times, separate instance rates, and body pitch/roll tracking.
...
Run temporary short and five-times-long rectangular/curved paths with visible knot markers. Hold orbit, cross each boundary, release, then repeat on an attached orbit. There must be no jumps, and release must stop immediately.
...
- Implementation: Partial; confirmed sampler defect
