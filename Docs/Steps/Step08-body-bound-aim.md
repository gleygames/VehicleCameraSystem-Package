# Step 08 — Body-bound aim

- **Difficulty:** Hard
- **Recommended agent:** Astra High (gpt-6-astra, high)
- **Why:** Exact body identity, articulation, group splits, and target lifecycle.
- **Audit baseline:** Not implemented; straight-orbit prerequisite exists
- **Prerequisites:** [Step 07](./Step07-longer-chains.md)
- **Requirement IDs:** DATA-01, ORBIT-04, AIM-03, AIM-05. Shared IDs cover this step's slice only; see [coverage](../RequirementCoverage.md).

## Startup checkpoint

**MANDATORY HARD CHECKPOINT — STOP FIRST.** Read this task's guidance and difficulty, then say:

> This is a Hard task: Step 08 — Body-bound aim. Recommended agent: Astra High. Please set the agent you want to use, then explicitly tell me to continue. I am paused before implementation and tests.
...
Current watch points are transformed through the lead body. There is no selectable exact-body binding or target-disconnected command result.
...
Bind watch targets to exact body instances, preserve straight orbit during articulation, and handle root-group split.

Original Edit Mode check: Validate binding identity with duplicate profile instances.

Original Play Mode check/observation: Articulate and detach a trailer; observe fixed orbit shape and target-disconnect response.

## Scope and acceptance outcomes

- Bind authored watch targets to exact registered body instances through the public API while orbit geometry remains in the straight lead reference.
- On target detachment, retain the registered root component, terminate a targeting operation as disconnected and restore root-group default aim.
- Establish binding/disconnection contracts here; POI travel and its complete terminal lifecycle are implemented and integration-tested in Step 9.
...
- Bind two bodies that share one profile; verify selecting one does not resolve to the other.
- Test missing/unreachable body references, retained-root selection after a split and root-default fallback without asset mutation.
...
- Articulate the selected trailer and verify actual camera aim follows its real transform while orbit positions remain unchanged relative to the lead.
- Detach the target and verify observable disconnection and fallback. Detach an unrelated body and verify the selected surviving target remains bound.
- Use explicit public bindings and two camera instances rather than calling only watch-point helper functions.
...
Register two identical-profile trailers with visibly different watch markers. Select one, articulate it, then detach it; watch the aim follow exactly that body and return to the root default.
...
- Implementation: Not implemented; straight-orbit prerequisite exists
