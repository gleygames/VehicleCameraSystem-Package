# Step 23 — Scene editing handles

- **Difficulty:** Hard
- **Recommended agent:** Astra High (gpt-6-astra, high)
- **Why:** Scene-space authoring, Undo, multiple data types, and runtime consistency.
- **Audit baseline:** Not implemented
- **Prerequisites:** [Step 22](./Step22-guided-profile-creation.md)
- **Requirement IDs:** DATA-01, EDIT-01. Shared IDs cover this step's slice only; see [coverage](../RequirementCoverage.md).

## Startup checkpoint

**MANDATORY HARD CHECKPOINT — STOP FIRST.** Read this task's guidance and difficulty, then say:

> This is a Hard task: Step 23 — Scene editing handles. Recommended agent: Astra High. Please set the agent you want to use, then explicitly tell me to continue. I am paused before implementation and tests.
...
Read [workflow](../ImplementationWorkflow.md), [status](../ImplementationStatus.md), [master plan](../ImplementationPlan.md), [design](../VehicleCameraSystemDesign.md) sections 3, 7, 10, [decision log](../../DecisionLog.md), and [C# conventions](../CSharpConventions.md). Read the [dated audit](../ImplementationAudit-2026-09-21.md) for recovery context.
...
Edit single-body curves, removable sections, watch markers, and zoom/height envelope visually.

Original Edit Mode check: Test asset edits, undo, and profile validity.

Original Play Mode check/observation: Run the edited profile and observe the expected camera path.

## Scope and acceptance outcomes

- Provide visual handles for orbit points, removable sections, watch markers and zoom/height envelope.
- Cover design-required orbit default poses and seat bounds, and editable advanced connectors consistent with Step 22's approved authoring layout.
- Make edits undoable and persist only intended authoring assets; keep vehicle prefabs unchanged.
- Split this step into separately authorized slices if necessary, with the parent incomplete until every handle group is delivered.
...
- Exercise each handle's edit operation with transformed bodies and orientation adjustments; verify correct local/world conversion and serialized asset updates.
- Undo/redo curve, section, target, envelope, default-pose, connector and seat-bound changes; verify restoration and validation diagnostics.
- Verify invalid edits identify the affected asset/view and unrelated assets/prefabs stay unchanged.
...
- Run a profile edited through the same editor edit operations; verify actual camera path, aim, offset limits, defaults and seat envelope match authored data.
- Verify an edited connector is consumed by the public attach API and an invalid affected view does not block a valid alternative.
...
Edit each handle group in Scene view, Undo/Redo the change, and run the profile. Compare the visible path, framing and envelopes with the camera behavior.
